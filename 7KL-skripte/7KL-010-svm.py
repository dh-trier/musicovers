...
Parameters:
Features tested: faces, objects (pca max. 10 components), hsv values with 3 max values for H channel and max, std, median vor S and V channels
...
targetdatafile = join(workdir, "8KL-daten", "8KL-010-svm.csv")
documentationfile = join(workdir, "8KL-daten", "8KL-010-svm.txt")
logfile = join(workdir, "8KL-daten", "8KL-log-010-svm.csv")
docstring = __doc__
...
if len(sys.argv) >= 2:
    ...
    kerneltype = "poly"  # linear|poly|rbf
...
def get_featurematrix(data):
    ...
    all_columns = [faces_zs,hmax1_zs,hmax2_zs,hmax3_zs,smax1_zs,smed_zs,sstd,vmax1_zs,vmed_zs,vstd,comp_1,comp_2,comp_3,comp_4,comp_5,comp_6,comp_7,comp_8,comp_9,comp_10] # ,comp_11,comp_12,comp_13,comp_14,comp_15,comp_16,comp_17,comp_18,comp_19,comp_20,comp_21,comp_22,comp_23,comp_24,comp_25
...
def define_classifier(classifiertype, kernel):
    """
    ...
    if classifiertype == "svm":
        classifier = svm.SVC(kernel=kernel)  # linear|poly|rbf
...
def log_scores(score):
    logstr = "{},{},{}\n".format(classifiertype, kerneltype, score)
...
def main(sourcedatafile, targetdatafile, documentationfile, classifiertype, tail):
    ...
    classifier = define_classifier(classifiertype, kerneltype)
